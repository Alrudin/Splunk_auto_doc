...
def check_database_ready(db_params: dict[str, str]) -> bool:
    ...
    try:
        ...
        engine = create_engine(
            url, echo=False, pool_pre_ping=True, connect_args={"connect_timeout": 5}
        )
...
def wait_for_database(max_retries: int = 30, retry_interval: int = 2) -> bool:
    ...
    print(
        f"❌ ERROR: PostgreSQL did not become ready within {total_time} seconds",
        file=sys.stderr,
    )
    print("", file=sys.stderr)
    print("Troubleshooting steps:", file=sys.stderr)
    print(
        "  1. Check if PostgreSQL container is running: docker compose ps db",
        file=sys.stderr,
    )
    ...
    print(
        "  4. Ensure PostgreSQL health check is configured in docker-compose.yml",
        file=sys.stderr,
    )
    print("", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print verbose error messages"
    )
