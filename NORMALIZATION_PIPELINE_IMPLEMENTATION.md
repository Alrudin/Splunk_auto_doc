...
- 1,000 stanzas: < 5 seconds ✅
- 10,000 stanzas: < 10 seconds ✅ (meets requirement)
...
3. **SQLite Compatibility:** Bulk insert using SQLAlchemy Core works with both PostgreSQL (production) and SQLite (testing). Some PostgreSQL-specific features (like JSONB operators) are used in queries but not in inserts.
